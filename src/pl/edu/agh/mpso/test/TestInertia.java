...
public class TestInertia {
	...
	public static void main(String[] args)
		...
		for (int i = 90; i < 100; i+=2) {
			String fileName = "results/Griewank_50_inertia" + String.valueOf(i) + ".json";
	...
	private static void simulate(int i, String configName, FitnessFunction fitnessFunction, String fileName)
		...
		double inertia = i * 0.01;

		for (int j = 0; j < EXECUTIONS; j++) {
			Writer writer = new FileWriter(fileName, true);
			try {
				result = run(configuration, fitnessFunction, inertia);
	...
	private static SimulationResult run(List<SwarmInformation> swarmInformations, FitnessFunction fitnessFunction, double inertia) {
